 ...
 * PARAMETER:    N/A
 ********************************************/
//the additional bit (Boolean for red/Black) that is
//added to each node is to ensure the balance during insertions and deletions.
template <class T>
...
class BST<T>::BNode {
   public:
    ...
    pRight{nullptr}, isRed {false} {} //added default value for isRed
      BNode(const T & element) : data{element}, pParent{nullptr},
    pLeft{nullptr}, pRight{nullptr}, isRed {false} {} //added default value for isRed
    ...
    bool isRed;   // isRed is here
    bool validRB(BNode *pNode, int depth);

   private:
    int depth;
    int lDepth;
    int rDepth;
    int verifyRB(BNode *pNode, int depth);
    void verifyBST();
    int findDepth(BNode *pNode);
    void balance();
};
/*******************************************************
...
********************************************************/
template<class T>
int BST<T> :: BNode ::  findDepth (BNode *pNode)
{
    try
    {
        if (pNode == NULL)
        {
            throw ("This structure has no nodes.");
        } else
        {
            lDepth = findDepth(pNode->pLeft); //recursion won't work in this project
            // how to include existing iterator here?
            // call find()???
            rDepth = findDepth(pNode->pRight);
    ...
    {
        std::cout << "findDepth threw an exception";
    }
    return depth;
}
/*******************************************************
* verifyRB(int depth)
* the heights of the two subtrees differ at most by one
* as adapted from www.stackoverflow.com/questions/
* 27731072
********************************************************/
// return_type class_name :: function_name (parameters)
// this or a similar function needs to return a bool
template <class T>
int BST<T> :: BNode :: verifyRB (BNode *pNode, int depth)
{
    findDepth (pNode);
    int colorBit = pNode->isRed == isRed ? 1 : 0;
    if (pNode->isRed == isRed) //isRed comprises a property of BNode
    {
        return 1;
    } else
    {
    ...
    if (lDepth == -1 || rDepth == -1 || lDepth != rDepth)
    {
        return -1;
    } else
    {
        return lDepth + colorBit;
    }
}
/*******************************************************
* validRB()
********************************************************/
template <class T>
bool BST<T> :: BNode :: validRB (BNode *pNode, int depth)
{
    return verifyRB(pNode, depth);
}
/*******************************************************
* verifyBST()
********************************************************/
